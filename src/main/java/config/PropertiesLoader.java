...
class PropertiesLoader {
  ...
  /**
   * Static initialization block that loads the configuration files when the class is first loaded.
   * This ensures configurations are available before any subclass methods are called.
   */
  static {
  ...
  /**
   * Protected constructor to prevent direct instantiation of this utility class. Subclasses can
   * call this constructor, but the class is primarily designed for static method access.
   */
  protected PropertiesLoader() {}

  /**
   * Loads the main configuration from config.properties located in resources. This method is called
  ...
  /**
   * Loads the Petri net configuration based on the petrinet.number property. This method determines
   * which Petri net file to load and loads it into the petrinet Properties object.
   *
   * <p>The method reads "petrinet.number" from the main config to determine which Petri net
   * configuration to load, then loads the file specified by the "petrinet.{number}" property.
   *
   * @throws RuntimeException if the petri net file cannot be found, if the petrinet.number property
  ...
  /**
   * Retrieves a boolean value from the main configuration. Only accepts "true" or "false"
   * (case-insensitive) as valid values.
   *
  ...
  /**
   * Retrieves an integer value from the main configuration. Whitespace in the property value is
  ...
  /**
   * Retrieves a double value from the main configuration. Whitespace in the property value is
   * automatically trimmed.
   *
   * @param key the configuration key to retrieve
  ...
  /**
   * Retrieves an integer array from the Petri net configuration. Values should be comma-separated
   * in the properties file. Whitespace around values is automatically trimmed.
   *
   * @param key the configuration key to retrieve
