...
```bash
# Build the image
docker build -t concurrency-monitor .

# Run the container (one time)
docker run -p 8080:8080 -v $(pwd)/LOG.txt:/app/LOG.txt concurrency-monitor

# Run interactively to see output
docker run -it -p 8080:8080 -v $(pwd)/LOG.txt:/app/LOG.txt concurrency-monitor
```
...
docker exec -it concurrency-monitor bash

# View container logs
docker logs concurrency-monitor

# Stop the container
docker stop concurrency-monitor
```
...
```bash
# Run interactively
docker run -it --rm -v $(pwd)/LOG.txt:/app/LOG.txt concurrency-monitor
```